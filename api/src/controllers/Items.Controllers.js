...
export const getItem = async (req, res) => {
  ...
  try {
    ...
    console.error(error)
    return res.status(500).send('Error al obtener el ítem')
  }
}
...
export const updateItem = async (req, res) => {
  const { id, nombre, descripcion, estado, placa, serial, company } = req.body
  ...
  try {
    ...
    const item = await ItemModel.findOneAndUpdate({ _id: id }, { descripcion, estado, nombre, placa, serial }, { new: true })
